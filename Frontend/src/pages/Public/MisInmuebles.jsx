  ...
  const handleSubirTitulo = async (e) => {
    ...
    try {
      ...
      if (data.estado === 'verificado') {
        showAlert({
          title: '¡Título de Propiedad Verificado!',
          message: data.resumen_publico || 'El título de propiedad fue validado con éxito y el activo se selló automáticamente en la Blockchain.',
          status: 'success'
        });
      } else if (data.estado === 'observado') {
        showAlert({
          title: 'Título con Observaciones Registrales',
          message: data.resumen_publico || 'El documento analizado presenta gravámenes u observaciones registrales.',
          status: 'warning'
        });
      } else if (data.estado === 'rechazado') {
        showAlert({
          title: 'Documento No Válido / Rechazado',
          message: data.resumen_publico || 'El documento subido no corresponde a un título de propiedad inmobiliaria válido o no coincide con el propietario registrado.',
          status: 'error'
        });
      } else {
        showAlert({
          title: 'Análisis Legal Procesado',
          message: data.resumen_publico || 'El análisis del documento ha finalizado.',
          status: 'info'
        });
      }
      fetchData();
